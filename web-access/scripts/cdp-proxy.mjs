...
// --- WebSocket 兼容层 ---
let WS;
// 优先使用 ws 模块（Node 22 原生 WebSocket 连接 CDP 有兼容性问题）
try {
  ...
  if (typeof globalThis.WebSocket !== 'undefined') {
    ...
    console.error('[CDP Proxy] 错误：未安装 ws 模块');
    console.error('  解决方案：cd ~/.claude/skills/web-access && npm install ws');
    process.exit(1);
  }
}
...
async function discoverChromePort() {
  ...
  // 2. 扫描常用端口，并通过 HTTP API 获取 wsPath
  const commonPorts = [9222, 9229, 9333];
  for (const port of commonPorts) {
    const ok = await checkPort(port);
    if (ok) {
      // 通过 HTTP API 获取正确的 webSocketDebuggerUrl
      try {
            ...
            res.on('end', () => {
              try {
                ...
                if (info.webSocketDebuggerUrl) {
                  const parsed = new URL(info.webSocketDebuggerUrl);
        ...
        console.log(`[CDP Proxy] 扫描发现 Chrome 调试端口: ${port}${wsUrl ? ' (带 wsPath)' : ''}`);
        return { port, wsPath: wsUrl };
      } catch {
        console.log(`[CDP Proxy] 端口 ${port} 有响应但不是 Chrome CDP，跳过`);
        continue;
      }
    }
  }
